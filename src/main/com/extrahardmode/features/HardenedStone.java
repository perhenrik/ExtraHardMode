        ...
        if (hardStoneEnabled && block.getType() == Material.STONE &&! playerBypasses)
        {
            ...
            if (inHandStack != null)
            {
                ...
                else if (hardEvent.getNumOfBlocks() > 0)
                {
                    ...
                    double damagePerBlock = maxDurability / hardEvent.getNumOfBlocks();

                    if (maxDurability > 0 && damagePerBlock > 0.0)
                    {
                        int durability =  inHandStack.getDurability();
                        durability += damagePerBlock;
                        //High numbers like 800 cause it's an int
                        double percentChance = (maxDurability % hardEvent.getNumOfBlocks()) / (double)maxDurability;
                        if (plugin.getRandom().nextDouble() < percentChance)
                            durability += damagePerBlock;
                        durability += 10;
                        plugin.getServer().broadcastMessage("" + durability);
                        inHandStack.setDurability((short) durability);
